...
class MapillaryDataset(utils.Dataset):
    ...
    CLASSES = ["Bird", "Person", "Bicyclist", "Motorcyclist", "Bench",          "Fire Hydrant", "Traffic Light", "Bus", "Car", "Motorcycle", "Truck"]
    ...
    def load_mapillary(self, dataset_dir, subset, class_ids=None,
        ...
        if (self.DEBUG):
            print("{} images in directory {}".format(len(file_names), self.IMG_DIR))
...
AWS_run_2_dir = os.path.join(DATASET_DIR, "AWS_run_3")

# Training dataset
dataset_train = MapillaryDataset()
dataset_train.load_mapillary(AWS_run_2_dir, "train_16384", class_ids = labels)
dataset_train.prepare()

# Validation dataset
dataset_val = MapillaryDataset()
dataset_val.load_mapillary(AWS_run_2_dir, "dev_1024", class_ids = labels)
dataset_val.prepare()
...
print("{} images in dataset {}".format(dataset_train.IMG_DIR, len(dataset_train.image_ids)))
print("{} images in dataset {}".format(dataset_val.IMG_DIR, len(dataset_val.image_ids)))
